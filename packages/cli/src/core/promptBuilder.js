...
exports.buildSingleTaskPrompt = buildSingleTaskPrompt;
exports.buildTaskBoxPrompt = buildTaskBoxPrompt;
exports.buildBundleTaskPrompt = buildBundleTaskPrompt;
exports.buildPersonaContext = buildPersonaContext;
exports.buildDebugPrompt = buildDebugPrompt;
exports.appendPromptTail = appendPromptTail;
/**
 * Build a rich context prompt for a single task.
 */
function buildSingleTaskPrompt(t, lane) {
    const sections = [];
    ...
    // Metadata
    if (t.targetRole) {
        sections.push(`Role: ${t.targetRole}`);
    }
    if (t.priority && t.priority !== 5) {
        sections.push(`Priority: ${t.priority}/10`);
    }
    // Swim lane context
    if (lane) {
        sections.push(``);
        ...
        if (effectiveDir) {
            sections.push(`Working Directory: ${effectiveDir}`);
        }
        if (t.serverOverride) {
            sections.push(`Server: ${t.serverOverride}`);
        }
 ...
 */
function buildTaskBoxPrompt(parent, subtasks, lane) {
    const sections = [];
    ...
    for (let i = 0; i < subtasks.length; i++) {
        ...
        if (sub.input) {
            sections.push(`Description / Details:\n${sub.input}`);
        }
        if (sub.targetRole) {
            sections.push(`Role: ${sub.targetRole}`);
        }
    }
    ...
    if (lane) {
        ...
        if (parent.serverOverride) {
            sections.push(`Server: ${parent.serverOverride}`);
        }
        if (lane.contextInstructions) {
            sections.push(`\nContext / Instructions:\n${lane.contextInstructions}`);
        }
 ...
 */
function buildBundleTaskPrompt(t, otherTasks, lane) {
    const sections = [];
 ...
 */
function buildDebugPrompt(lane) {
    const sections = [];
    ...
    if (lane.workingDirectory) {
        sections.push(`Working Directory: ${lane.workingDirectory}`);
    }
    if (lane.contextInstructions) {
        sections.push(``);
 ...
 */
function appendPromptTail(prompt, options) {
