...
function evaluateAgentState(currentState, aiProvider, paneContent, aiAssistant) {
    ...
    switch (aiStatus) {
        case types_1.AIStatus.WORKING:
            return types_1.AgentState.WORKING;
        case types_1.AIStatus.WAITING:
        case types_1.AIStatus.IDLE:
            return types_1.AgentState.IDLE;
        default:
            return currentState;
    }
}
class AgentOrchestrator {
    constructor() {
        this.agents = new Map();
        this.taskQueue = [];
        this.aiAssistantManager = new aiAssistant_1.AIAssistantManager();
        this._onAgentStateChanged = new eventEmitter_1.EventEmitter();
        this.onAgentStateChanged = this._onAgentStateChanged.event;
        this._onTaskCompleted = new eventEmitter_1.EventEmitter();
        this.onTaskCompleted = this._onTaskCompleted.event;
        this._onPipelineEvent = new eventEmitter_1.EventEmitter();
        this.onPipelineEvent = this._onPipelineEvent.event;
        this._onAgentMessage = new eventEmitter_1.EventEmitter();
    ...
    removeAgent(agentId) {
        ...
        if (agent) {
            agent.state = types_1.AgentState.TERMINATED;
            this._onAgentStateChanged.fire(agent);
    ...
    getIdleAgents(role) {
        return this.getAllAgents().filter(a => {
            if (a.state !== types_1.AgentState.IDLE) {
                return false;
            }
    ...
    async dispatchNextTask() {
        ...
        const pendingTask = this.taskQueue.find(t => t.status === types_1.TaskStatus.PENDING);
        if (!pendingTask) {
            return;
        }
        ...
        // Assign the task
        pendingTask.status = types_1.TaskStatus.ASSIGNED;
        pendingTask.assignedAgentId = agent.id;
        pendingTask.startedAt = Date.now();
        agent.currentTaskId = pendingTask.id;
        this.updateAgentState(agent.id, types_1.AgentState.WORKING);
        try {
            await this.sendPromptToAgent(agent.id, pendingTask.description);
            pendingTask.status = types_1.TaskStatus.IN_PROGRESS;
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            pendingTask.status = types_1.TaskStatus.FAILED;
            pendingTask.errorMessage = errorMessage;
            this.updateAgentState(agent.id, types_1.AgentState.ERROR, errorMessage);
        }
    }
    // ─── Agent Communication ────────────────────────────────────────────
    ...
    getFanOutResults(stageId) {
        return this.taskQueue.filter(t => t.pipelineStageId === stageId && t.status === types_1.TaskStatus.COMPLETED);
    }
    // ─── Agent Messaging ────────────────────────────────────────────────
    ...
    async checkAgentStates(serviceManager) {
        ...
        for (const agent of agents) {
            if (agent.state === types_1.AgentState.TERMINATED || agent.state === types_1.AgentState.COMPLETED) {
                continue;
            }
            ...
            try {
                ...
                if (ccState) {
                    ...
                    if (aiStatus !== null) {
                        ...
                        switch (aiStatus) {
                            case types_1.AIStatus.WORKING:
                                newState = types_1.AgentState.WORKING;
                                break;
                            case types_1.AIStatus.WAITING:
                            case types_1.AIStatus.IDLE:
                                newState = types_1.AgentState.IDLE;
                                break;
                            default:
                ...
                this.updateAgentState(agent.id, types_1.AgentState.ERROR, 'Failed to read pane content');
            }
        }
    }
    updateAgentState(agentId, newState, error) {
        ...
        if (newState === types_1.AgentState.IDLE && agent.currentTaskId) {
