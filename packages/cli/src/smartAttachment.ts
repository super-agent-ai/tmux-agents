...
import { AttachmentStrategy, AttachmentResult } from './core/types';
...
export class SmartAttachmentService {
    ...
    resolveStrategy(
        sessionName: string,
        serverId: string,
        existingTerminals: readonly vscode.Terminal[]
    ): AttachmentResult {
    ...
    async attachToSession(
        service: TmuxService,
        sessionName: string,
        options?: { windowIndex?: string; paneIndex?: string }
     ...
     */
    generateTerminalName(baseName: string, existingNames: string[]): string {
