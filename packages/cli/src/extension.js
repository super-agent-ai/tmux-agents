...
const apiCatalog_1 = require("./apiCatalog");
const serviceManager_1 = require("./serviceManager");
const smartAttachment_1 = require("./smartAttachment");
const aiAssistant_1 = require("./aiAssistant");
const hotkeyManager_1 = require("./hotkeyManager");
const agentTemplate_1 = require("./agentTemplate");
...
const sessionCommands_1 = require("./commands/sessionCommands");
const agentCommands_1 = require("./commands/agentCommands");
const autoMonitor_1 = require("./autoMonitor");
...
function getServiceForItem(serviceManager, item) {
    let serverId;
...
async function pickService(serviceManager) {
    ...
    const choice = await vscode.window.showQuickPick(services.map(s => ({ label: s.serverLabel, description: s.serverId, service: s })), { placeHolder: 'Select server' });
    return choice?.service;
}
function buildTaskWindowName(task) {
...
console.log = (...args) => { log(args.map(String).join(' ')); _origLog(...args); };
console.warn = (...args) => { log('[WARN] ' + args.map(String).join(' ')); _origWarn(...args); };
console.error = (...args) => { log('[ERROR] ' + args.map(String).join(' ')); _origError(...args); };
function activate(context) {
    ...
    const serviceManager = new serviceManager_1.TmuxServiceManager();
    const tmuxSessionProvider = new treeProvider_1.TmuxSessionProvider(serviceManager, context.extensionPath);
    const smartAttachment = new smartAttachment_1.SmartAttachmentService();
    const aiManager = new aiAssistant_1.AIAssistantManager();
    ...
    const shortcutsProvider = new treeProvider_1.ShortcutsProvider();
    vscode.window.registerTreeDataProvider('tmux-agents-shortcuts', shortcutsProvider);
    // ── Orchestration System ─────────────────────────────────────────────────
    const orchestrator = new core_1.AgentOrchestrator();
    orchestrator.setServiceManager(serviceManager);
    const taskRouter = new core_1.TaskRouter();
    const pipelineEngine = new core_1.PipelineEngine();
    const templateManager = new agentTemplate_1.AgentTemplateManager();
    const teamManager = new teamManager_1.TeamManager();
    const organizationManager = new organizationManager_1.OrganizationManager();
    const guildManager = new guildManager_1.GuildManager();
    // ── Default Prompt Templates ─────────────────────────────────────────────
    const promptRegistry = new core_1.PromptRegistry(context.extensionPath);
    const defaultPromptsEnabled = vscode.workspace.getConfiguration('tmuxAgents').get('defaultPromptsEnabled', true);
    if (defaultPromptsEnabled) {
        promptRegistry.load(context.extensionPath);
    }
    const promptExecutor = new core_1.PromptExecutor();
    // ── Database ──────────────────────────────────────────────────────────────
    const dbDir = context.globalStorageUri.fsPath;
    const dbPath = require('path').join(dbDir, 'tmux-agents.db');
    const database = new core_1.Database(dbPath);
    // ── API Catalog & Chat View ──────────────────────────────────────────────
    const apiCatalog = new apiCatalog_1.ApiCatalog({
        ...
        deleteSwimLane: (id) => {
            ...
            if (idx !== -1) {
                swimLanes.splice(idx, 1);
                database.deleteSwimLane(id);
            }
        },
    ...
    const chatViewProvider = new chatView_1.ChatViewProvider(serviceManager, context.extensionUri, apiCatalog, { orchestrator, teamManager, pipelineEngine, templateManager }, aiManager);
    ...
    const dashboardView = new dashboardView_1.DashboardViewProvider(context.extensionUri);
    const graphView = new graphView_1.GraphViewProvider(context.extensionUri);
    const kanbanView = new kanbanView_1.KanbanViewProvider(context.extensionUri);
    const swimLanes = [];
    ...
    const orchCfg = orchestratorConfig.get('orchestrator') || {};
        ...
        if (task.parentTaskId) {
            ...
            if (parent && parent.subtaskIds) {
                const allDone = parent.subtaskIds.every(sid => {
                    ...
                    return sub && sub.status === core_1.TaskStatus.COMPLETED;
                });
                if (allDone) {
...
If any subtask output shows errors, test failures, or incomplete work, the verdict should be FAIL.`;
                                ...
                                parent.status = core_1.TaskStatus.COMPLETED;
                                parent.completedAt = Date.now();
                                (0, autoCloseMonitor_1.markDoneTimestamp)(parent);
                                database.saveTask(parent);
                            }
                        }
                    }
    ...
    const graphActionDisposable = graphView.onAction(({ action, payload }) => {
        switch (action) {
            ...
            case 'addStage':
                ...
                if (pipeline) {
                    pipelineEngine.addStage(payload.pipelineId, {
                        name: 'New Stage',
                        type: core_1.StageType.SEQUENTIAL,
                        agentRole: core_1.AgentRole.CODER,
            ...
            case 'removeStage':
                ...
                if (updatedPipeline) {
                    graphView.setPipeline(updatedPipeline);
                }
                break;
        }
    });
    // ── startTaskFlow: reusable function to launch a task in tmux ──────────
    async function startTaskFlow(t, options) {
        ...
        if (lane) {
            ...
            try {
                ...
                const winIndex = win?.index || '0';
                const paneIndex = win?.panes[0]?.index || '0';
                if ((0, core_1.resolveToggle)(t, 'useWorktree', lane) && effectiveWorkingDir) {
                ...
                if (t.subtaskIds && t.subtaskIds.length > 0) {
                    const subtasks = t.subtaskIds.map(id => orchestrator.getTask(id)).filter((s) => !!s);
                    prompt = (0, core_1.buildTaskBoxPrompt)(t, subtasks, lane);
                    for (const sub of subtasks) {
                        sub.kanbanColumn = 'in_progress';
                        sub.status = core_1.TaskStatus.IN_PROGRESS;
                    ...
                    prompt = (0, core_1.buildSingleTaskPrompt)(t, lane);
                }
                ...
                if (t.assignedAgentId) {
                    ...
                    if (agent?.persona) {
                        personaContext = (0, core_1.buildPersonaContext)(agent.persona);
                    }
                    if (agent) {
                ...
                let memoryLoadContext;
                let memorySaveContext;
                if ((0, core_1.resolveToggle)(t, 'useMemory', lane) && lane.memoryFileId) {
                    try {
                        await (0, core_1.ensureMemoryDir)(service, lane);
                        const memoryContent = await (0, core_1.readMemoryFile)(service, lane);
                        const memoryFilePath = (0, core_1.getMemoryFilePath)(lane);
                        memoryLoadContext = (0, core_1.buildMemoryLoadPrompt)(memoryContent, memoryFilePath);
                        memorySaveContext = (0, core_1.buildMemorySavePrompt)(memoryFilePath);
                    }
                    catch (err) {
                        console.warn('[Memory] Failed to load memory:', err);
                    }
                }
                prompt = (0, core_1.appendPromptTail)(prompt, {
                ...
                const isAutoPilot = (0, core_1.resolveToggle)(t, 'autoPilot', lane);
                const launchCmd = aiManager.getInteractiveLaunchCommand(resolvedProvider, resolvedModel, isAutoPilot);
                const launchDelay = vscode.workspace.getConfiguration('tmuxAgents').get('cliLaunchDelayMs', 3000);
                setTimeout(async () => {
                    try {
                        await service.sendKeys(lane.sessionName, winIndex, paneIndex, launchCmd);
                ...
                t.status = core_1.TaskStatus.IN_PROGRESS;
    ...
    const kanbanActionDisposable = kanbanView.onAction(async ({ action, payload }) => {
        await (0, kanbanHandlers_1.handleKanbanMessage)(action, payload, {
            serviceManager,
            tmuxSessionProvider,
            smartAttachment,
            aiManager,
            ...
            startTaskFlow,
            buildTaskWindowName,
            cleanupInitWindow: async (serverId, sessionName) => {
                const svc = serviceManager.getService(serverId);
                if (svc) {
                    await cleanupInitWindow(svc, sessionName);
                }
            },
        });
    });
    ...
    async function triggerDependentsInline(completedTaskId) {
        const allTasks = orchestrator.getTaskQueue();
        for (const t of allTasks) {
            if (!t.dependsOn || !t.dependsOn.includes(completedTaskId)) {
                continue;
            }
            const allMet = t.dependsOn.every(depId => {
                const dep = orchestrator.getTask(depId);
                return dep && dep.status === core_1.TaskStatus.COMPLETED;
            });
    ...
    (0, sessionSync_1.syncTaskListAttachments)(sessionSyncCtx).catch(err => console.warn('tmux-agents: Initial session sync failed:', err));
    const autoMonitorTimer = setInterval(async () => {
        await (0, autoMonitor_1.checkAutoCompletions)(autoMonitorCtx);
        await (0, autoMonitor_1.checkAutoPilot)(autoMonitorCtx);
        await (0, autoCloseMonitor_1.checkAutoCloseTimers)(autoCloseCtx);
        await (0, sessionSync_1.syncTaskListAttachments)(sessionSyncCtx);
    }, 15000);
    async function ensureLaneSession(lane) {
        const service = serviceManager.getService(lane.serverId);
        if (!service) {
    ...
    async function cleanupInitWindow(service, sessionName) {
        try {
            const sessions = await service.getTmuxTreeFresh();
    ...
    async function advancePipeline(runId) {
        ...
        for (const [stageId, result] of Object.entries(run.stageResults)) {
            if (result.status === core_1.TaskStatus.COMPLETED && result.output) {
    ...
    const sessionDisposables = (0, sessionCommands_1.registerSessionCommands)(context, {
        serviceManager,
        tmuxSessionProvider,
        smartAttachment,
        aiManager,
    ...
    const agentDisposables = (0, agentCommands_1.registerAgentCommands)(context, {
        serviceManager,
        tmuxSessionProvider,
        aiManager,
        orchestrator,
    ...
    const promptExecutorContext = {
        promptRegistry,
        submitTask: (task) => orchestrator.submitTask(task),
        saveTask: (task) => database.saveTask(task),
        startTaskFlow: (task, options) => startTaskFlow(task, options),
        swimLanes,
    };
    ...
    const executeDefaultPromptCmd = vscode.commands.registerCommand('tmux-agents.executeDefaultPrompt', async (slug) => {
        if (!defaultPromptsEnabled) {
            vscode.window.showWarningMessage('Default prompts are disabled. Enable tmuxAgents.defaultPromptsEnabled to use them.');
            return;
        }
        if (!slug) {
            const templates = promptRegistry.getAllTemplates();
            const picked = await vscode.window.showQuickPick(templates.map(t => ({ label: t.name, description: t.description, slug: t.slug })), { placeHolder: 'Select a prompt to execute' });
            if (!picked) {
                return;
            }
            slug = picked.slug;
        ...
        if (swimLanes.length > 0) {
            const lanePick = await vscode.window.showQuickPick([
                { label: '(none)', description: 'No swim lane', id: undefined },
                ...swimLanes.map(l => ({ label: l.name, description: l.workingDirectory, id: l.id })),
    ...
    // ── Register All ──────────────────────────────────────────────────────────
    context.subscriptions.push(...sessionDisposables, ...agentDisposables, 
    // Default prompt commands
    listDefaultPromptsCmd, executeDefaultPromptCmd, 
    // Event subscriptions
    agentStateChangedDisposable, taskCompletedDisposable, orgChangedDisposable, guildChangedDisposable, agentMessageDisposable, dashboardActionDisposable, graphActionDisposable, kanbanActionDisposable, 
    // Disposables
    { dispose: () => clearInterval(autoMonitorTimer) }, { dispose: () => database.close() }, outputChannel, serviceManager, tmuxSessionProvider, orchestrator, pipelineEngine, teamManager, organizationManager, guildManager, dashboardView, graphView, kanbanView, promptRegistry);
