...
import { AIProvider, AgentInstance, AgentState, AgentRole, AgentTemplate, StageType, Pipeline, PipelineRun } from '../core/types';
...
async function spawnAgentFromTemplate(
    ...
    try {
        ...
        setTimeout(() => {
            ctx.orchestrator.updateAgentState(agentId, AgentState.IDLE);
        }, 3000);
...
export function registerAgentCommands(
    ...
    disposables.push(vscode.commands.registerCommand('tmux-agents.sendToAgent', async () => {
        const agents = ctx.orchestrator.getAllAgents().filter(a => a.state !== AgentState.TERMINATED);
        if (agents.length === 0) {
            vscode.window.showInformationMessage('No active agents.');
            return;
        }
    ...
    disposables.push(vscode.commands.registerCommand('tmux-agents.fanOut', async () => {
        ...
        const templates = ctx.templateManager.getTemplatesByRole(AgentRole.CODER);
    ...
    disposables.push(vscode.commands.registerCommand('tmux-agents.quickTeamCoding', async () => {
        const coderTemplate = ctx.templateManager.getTemplatesByRole(AgentRole.CODER)[0];
        const reviewerTemplate = ctx.templateManager.getTemplatesByRole(AgentRole.REVIEWER)[0];
        const testerTemplate = ctx.templateManager.getTemplatesByRole(AgentRole.TESTER)[0];
        ...
        for (const tmpl of [coderTemplate, reviewerTemplate, testerTemplate]) {
            ...
            if (agent) { ctx.teamManager.addAgentToTeam(team.id, agent.id); }
        }
        const crPipeline = ctx.pipelineEngine.getAllPipelines().find(p => p.name === 'Code Review Pipeline');
        if (crPipeline) { ctx.teamManager.setPipelineForTeam(team.id, crPipeline.id); }
        ctx.updateDashboard();
    ...
    disposables.push(vscode.commands.registerCommand('tmux-agents.quickTeamResearch', async () => {
        const researcherTemplate = ctx.templateManager.getTemplatesByRole(AgentRole.RESEARCHER)[0];
        const coderTemplate = ctx.templateManager.getTemplatesByRole(AgentRole.CODER)[0];
        ...
        if (riPipeline) { ctx.teamManager.setPipelineForTeam(team.id, riPipeline.id); }
        ctx.updateDashboard();
