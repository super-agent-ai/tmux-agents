...
exports.triggerDependents = triggerDependents;
exports.handleKanbanMessage = handleKanbanMessage;
...
async function triggerDependents(ctx, completedTaskId) {
...
async function handleKanbanMessage(action, payload, ctx) {
    switch (action) {
        case 'browseDir': {
            ...
            if (!browseService || browseService.serverIdentity.isLocal) {
                ...
                while (true) {
                    let dirs;
                    try {
                        const raw = await browseService.execCommand(`cd ${currentPath.replace(/"/g, '\\"')} 2>/dev/null && pwd && find . -maxdepth 1 -type d ! -name . -printf '%f\\n' 2>/dev/null | sort || ls -1p | grep '/$' | sed 's/\\/$//'`);
            ...
            try {
                ...
                if (!win) {
                    ...
                    if (win) {
                        ...
                        const debugPrompt = (0, promptBuilder_1.buildDebugPrompt)(lane);
                        const capturedSession = lane.sessionName;
                        const capturedWin = win.index;
                        const capturedPane = pIdx;
                        const debugDelay = vscode.workspace.getConfiguration('tmuxAgents').get('cliLaunchDelayMs', 3000);
                        setTimeout(async () => {
                            try {
        ...
        case 'restartDebugWindow': {
            ...
            try {
                ...
                if (win) {
                    ...
                    // Auto-insert swim lane instructions after CLI is ready
                    const restartDebugPrompt = (0, promptBuilder_1.buildDebugPrompt)(lane);
                    const rCapturedSession = lane.sessionName;
                    const rCapturedWin = win.index;
                    const rCapturedPane = pIdx;
                    const restartDelay = vscode.workspace.getConfiguration('tmuxAgents').get('cliLaunchDelayMs', 3000);
                    setTimeout(async () => {
                        try {
        ...
        case 'createTask': {
            ...
            if (payload.autoStart !== undefined) {
                task.autoStart = !!payload.autoStart;
            }
            if (payload.autoPilot !== undefined) {
                task.autoPilot = !!payload.autoPilot;
            }
            if (payload.autoClose !== undefined) {
                task.autoClose = !!payload.autoClose;
            }
            if (payload.useWorktree !== undefined) {
                task.useWorktree = !!payload.useWorktree;
            }
            // Then inherit swim lane defaults for any toggles not explicitly set
            if (task.swimLaneId) {
                const lane = ctx.swimLanes.find(l => l.id === task.swimLaneId);
                (0, types_1.applySwimLaneDefaults)(task, lane);
            }
        ...
        case 'moveTask': {
            ...
            if (t) {
                ...
                if (oldColumn === 'done' && payload.kanbanColumn !== 'done') {
                    (0, autoCloseMonitor_1.cancelAutoClose)(t);
                }
                if (payload.kanbanColumn === 'done') {
                    t.status = types_1.TaskStatus.COMPLETED;
                    t.completedAt = Date.now();
                    (0, autoCloseMonitor_1.markDoneTimestamp)(t);
                    ...
                    if (t.worktreePath && t.tmuxServerId) {
                        ...
                        if (svc) {
                            try {
                                ...
                                console.warn('[Kanban] Failed to remove worktree:', err);
                            }
                        }
                        ...
                        if (ready) {
                            ...
                            if (service) {
                                try {
                                    ...
                                    await service.newWindow(lane.sessionName, windowName);
                                    await ctx.cleanupInitWindow(lane.serverId, lane.sessionName);
                                    t.status = types_1.TaskStatus.IN_PROGRESS;
                ...
                if (t && payload.kanbanColumn === 'todo' && t.swimLaneId) {
                    ...
                    if ((0, types_1.resolveToggle)(t, 'autoStart', moveLane)) {
        ...
        case 'editTask': {
            ...
            if (t && payload.updates) {
                if (payload.updates.description !== undefined)
                    t.description = payload.updates.description;
                if (payload.updates.input !== undefined)
                    t.input = payload.updates.input;
                if (payload.updates.targetRole !== undefined)
                    t.targetRole = payload.updates.targetRole;
                if (payload.updates.priority !== undefined)
                    t.priority = payload.updates.priority;
                if (payload.updates.autoStart !== undefined)
                    t.autoStart = !!payload.updates.autoStart;
                if (payload.updates.autoPilot !== undefined)
                    t.autoPilot = !!payload.updates.autoPilot;
                if (payload.updates.autoClose !== undefined)
                    t.autoClose = !!payload.updates.autoClose;
                if (payload.updates.useWorktree !== undefined)
                    t.useWorktree = !!payload.updates.useWorktree;
                if (payload.updates.aiProvider !== undefined)
                    t.aiProvider = payload.updates.aiProvider || undefined;
                if (payload.updates.aiModel !== undefined)
                    t.aiModel = payload.updates.aiModel || undefined;
                if (payload.updates.serverOverride !== undefined)
                    t.serverOverride = payload.updates.serverOverride || undefined;
                if (payload.updates.workingDirectoryOverride !== undefined)
                    t.workingDirectoryOverride = payload.updates.workingDirectoryOverride || undefined;
                if (payload.updates.dependsOn !== undefined)
                    t.dependsOn = payload.updates.dependsOn;
            }
            if (t) {
                ctx.database.saveTask(t);
            }
        ...
        case 'startTask': {
            ...
            if (payload.toggles) {
                if (payload.toggles.autoStart !== undefined)
                    t.autoStart = !!payload.toggles.autoStart;
                if (payload.toggles.autoPilot !== undefined)
                    t.autoPilot = !!payload.toggles.autoPilot;
                if (payload.toggles.autoClose !== undefined)
                    t.autoClose = !!payload.toggles.autoClose;
                if (payload.toggles.useWorktree !== undefined)
                    t.useWorktree = !!payload.toggles.useWorktree;
                ctx.database.saveTask(t);
            }
            await ctx.startTaskFlow(t, {
        ...
        case 'closeTaskWindow': {
            const t = ctx.orchestrator.getTask(payload.taskId);
            if (!t || !t.tmuxSessionName || !t.tmuxWindowIndex || !t.tmuxServerId)
                break;
            const svc = ctx.serviceManager.getService(t.tmuxServerId);
            if (!svc)
                break;
        ...
        case 'cleanupWorktree': {
            ...
            const isDone = t.kanbanColumn === 'done' || t.status === types_1.TaskStatus.COMPLETED;
            ...
            if (isDone) {
                ...
                const confirm = await vscode.window.showInformationMessage(`Cleanup worktree for completed task: rebase branch "${branchName}" onto main, merge, and remove worktree?`, { modal: true }, 'Rebase & Merge');
                if (confirm !== 'Rebase & Merge')
                    break;
                try {
                ...
                const confirm = await vscode.window.showWarningMessage(`Discard worktree for incomplete task? This will remove branch "${branchName}" and all uncommitted changes.`, { modal: true }, 'Discard Worktree');
                if (confirm !== 'Discard Worktree')
                    break;
                try {
        ...
        case 'startBundle': {
            ...
            const bundleTasks = taskIds.map(id => ctx.orchestrator.getTask(id)).filter((t) => !!t);
            ...
            if (lane) {
                ...
                for (const t of bundleTasks) {
                    try {
                        ...
                        let prompt = (0, promptBuilder_1.buildBundleTaskPrompt)(t, otherTasks, lane);
                        prompt = (0, promptBuilder_1.appendPromptTail)(prompt, {
                            additionalInstructions: payload.additionalInstructions,
                            askForContext: payload.askForContext,
                        ...
                        const isAutoPilot = (0, types_1.resolveToggle)(t, 'autoPilot', lane);
                        ...
                        const bundleDelay = vscode.workspace.getConfiguration('tmuxAgents').get('cliLaunchDelayMs', 3000);
                        setTimeout(async () => {
                            try {
                                await service.sendKeys(capturedSession, capturedWin, capturedPane, launchCmd);
                ...
                for (const pid of parentIds) {
                    ...
                    if (parent) {
                        ...
                        parent.status = types_1.TaskStatus.IN_PROGRESS;
        ...
        case 'mergeSelectedTasks': {
            const taskIds = payload.taskIds || [];
            const mergeTasks = taskIds.map(id => ctx.orchestrator.getTask(id)).filter((t) => !!t);
            if (mergeTasks.length < 2)
                break;
        ...
        case 'addSubtask': {
            ...
            ctx.database.saveTask(parentTask);
            if (childTask) {
                ctx.database.saveTask(childTask);
            }
            ctx.updateKanban();
        ...
        case 'editSwimLane': {
            ...
            if (payload.workingDirectory)
                lane.workingDirectory = payload.workingDirectory;
...
${content.slice(-3000)}`;
                    ...
                    proc.stdin.on('error', () => { });
                    process.nextTick(() => { if (proc.stdin && proc.stdin.writable && !proc.killed) {
...
- aiModel: string model name or "" to use the default. Only set if the user specifies a model.`;
                    if (genLane) {
                        ...
                        if (genLane.contextInstructions) {
                            prompt += `\n- Lane instructions: ${genLane.contextInstructions}`;
                        }
                        if (genLane.aiProvider) {
                        ...
                        if (dt) {
                            ...
                            if (dt.autoStart) {
                                toggleDefaults.push('autoStart=on');
                            }
                            if (dt.autoPilot) {
                                toggleDefaults.push('autoPilot=on');
                            }
                            if (dt.autoClose) {
                                toggleDefaults.push('autoClose=on');
                            }
                            if (dt.useWorktree) {
                                toggleDefaults.push('useWorktree=on');
                            }
...
4. Test Plan — concrete validation steps: commands to run, expected outputs, edge cases to check.`;
                    if (planLane) {
                        ...
                        if (planLane.contextInstructions) {
                ...
                if (planResult) {
                    ...
                    try {
                        ...
                        for (let i = 0; i < tasks.length; i++) {
                            if (!tasks[i].dependsOn) {
                                tasks[i].dependsOn = [];
                            }
                            tasks[i].dependsOn = tasks[i].dependsOn.filter((d) => typeof d === 'number' && d >= 0 && d < i);
                        }
        ...
        case 'approvePlan': {
            ...
            try {
                ...
                for (let i = 0; i < planTasks.length; i++) {
                    ...
                    if (approveLane) {
                        (0, types_1.applySwimLaneDefaults)(task, approveLane);
                    }
                    idMap.push(task.id);
                ...
                for (let i = 0; i < planTasks.length; i++) {
                    ...
                    if (deps.length > 0) {
                        createdTasks[i].dependsOn = deps
                            .filter((d) => d >= 0 && d < idMap.length && d !== i)
                            .map((d) => idMap[d]);
                    }
                }
                ...
                for (const task of createdTasks) {
                    const effectiveAutoStart = (0, types_1.resolveToggle)(task, 'autoStart', approveLane);
                    ...
                    for (const session of sessions) {
                        ...
                        for (const win of session.windows) {
                            if (win.panes.length > 0) {
                                ...
                                if (!primaryDir && pane.currentPath) {
                                    primaryDir = pane.currentPath;
                                }
                                try {
...
${combinedContent}`;
        ...
        case 'importTmuxSessions': {
            const sessionsToImport = payload.sessions || [];
            let importedWindows = 0;
            for (const s of sessionsToImport) {
                const windowsToImport = (s.selectedWindows || s.windows || []).filter((w) => !w.alreadyImported);
                if (windowsToImport.length === 0)
                    continue;
...
//# sourceMappingURL=kanbanHandlers.js.map