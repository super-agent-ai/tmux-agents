...
import { TmuxSession, TmuxWindow, TmuxPane, ServerIdentity, CcPaneMetadata,
         ...
         AIStatus, AI_STATUS_COLORS, AI_STATUS_ICONS, ActivityPriority } from './core/types';
import { ProcessTracker } from './core/processTracker';
import { AIAssistantManager } from './aiAssistant';
import { ActivityRollupService } from './core/activityRollup';
...
export class TmuxSessionProvider implements vscode.TreeDataProvider<TmuxTreeItem> {
    ...
    private async batchReadCcOptions(sessions: TmuxSession[]): Promise<Map<string, Record<string, string>>> {
        ...
        for (const [serverId, paneIds] of paneIdsByServer) {
            ...
            if (!service) { continue; }
            promises.push(
                service.getMultiplePaneOptions(paneIds).then((serverMap: Map<string, any>) => {
