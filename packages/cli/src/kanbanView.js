...
class KanbanViewProvider {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('tmux-agents-kanban', 'Kanban Board', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    updateState(tasks, swimLanes, servers, favouriteFolders) {
        this.tasks = tasks;
    ...
    sendState() {
        this.panel?.webview.postMessage({
            ...
            providerModels: aiModels_1.PROVIDER_MODELS
        });
    }
    getHtml() {
        return `<!DOCTYPE html>
...
.card-top-row {
    ...
    margin-bottom: 3px; gap: 4px; flex-wrap: wrap;
}
.card-id {
...
(function() {
    ...
    window.addEventListener('message', function(e) {
        ...
        if (msg.type === 'updateState') {
            ...
            if (msg.providerModels) { PROVIDER_MODELS = msg.providerModels; }
            renderFavBar();
            renderFilterTagsChips();
            render();
        }
