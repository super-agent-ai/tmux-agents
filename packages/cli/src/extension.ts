...
// Import from core/
import {
    TmuxService, AgentOrchestrator, TaskRouter, PipelineEngine, Database,
    buildSingleTaskPrompt, buildTaskBoxPrompt, appendPromptTail, buildPersonaContext,
    ensureMemoryDir, readMemoryFile, getMemoryFilePath, buildMemoryLoadPrompt, buildMemorySavePrompt,
    PromptRegistry, PromptExecutor,
    AIProvider, AgentRole, TaskStatus, OrchestratorTask, KanbanSwimLane, FavouriteFolder, StageType, resolveToggle
} from './core';
...
If any subtask output shows errors, test failures, or incomplete work, the verdict should be FAIL.`;
    ...
    async function cleanupInitWindow(service: TmuxService, sessionName: string): Promise<void> {
        try {
            const sessions = await service.getTmuxTreeFresh();
