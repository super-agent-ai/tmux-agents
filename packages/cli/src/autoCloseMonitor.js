...
exports.summarisePaneOutput = summarisePaneOutput;
exports.checkAutoCloseTimers = checkAutoCloseTimers;
exports.cancelAutoClose = cancelAutoClose;
exports.markDoneTimestamp = markDoneTimestamp;
 ...
 */
function summarisePaneOutput(raw) {
 ...
 */
async function checkAutoCloseTimers(ctx, delayMs = DEFAULT_DONE_DELAY_MS) {
    const allTasks = ctx.orchestrator.getTaskQueue();
    const candidates = allTasks.filter(t => t.kanbanColumn === 'done' &&
        t.doneAt &&
 ...
 */
async function closeTaskWindow(ctx, task) {
    ...
    let paneContent = '';
    try {
        paneContent = await service.capturePaneContent(task.tmuxSessionName, task.tmuxWindowIndex, task.tmuxPaneIndex || '0', 500);
    }
    catch {
    ...
    vscode.window.showInformationMessage(`Auto-closed tmux window for completed task: ${task.description.slice(0, 50)}`);
}
/**
 * Cancels a pending auto-close by clearing the `doneAt` timestamp.
 * Call this whenever a task is moved *out of* the 'done' column.
 */
function cancelAutoClose(task) {
 ...
 */
function markDoneTimestamp(task) {
    if (task.tmuxSessionName && task.tmuxWindowIndex && task.tmuxServerId) {
        task.doneAt = Date.now();
    }
}
//# sourceMappingURL=autoCloseMonitor.js.map