...
import { Guild, GuildKnowledge } from './core/types';

function generateId(): string {
    return crypto.randomUUID?.() || 'id-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
}

export class GuildManager implements vscode.Disposable {
    private guilds: Map<string, Guild> = new Map();

    private readonly _onGuildChanged = new vscode.EventEmitter<Guild>();
    public readonly onGuildChanged: vscode.Event<Guild> = this._onGuildChanged.event;
