...
suite('Command Registration — Auto-Registered', () => {
    ...
    test('tmux-agents-chat.focus is registered (webview view provider)', () => {
        assert.ok(
            registeredCommands.includes('tmux-agents-chat.focus'),
            'tmux-agents-chat.focus should be auto-registered by VS Code'
        );
    });
});
...
suite('Command Registration — Completeness', () => {
    ...
    test('Bug regression: tmux-agents.newSession does NOT exist (correct is tmux-agents.new)', () => {
        assert.ok(
            !registeredCommands.includes('tmux-agents.newSession'),
            'tmux-agents.newSession should NOT be registered — the correct ID is tmux-agents.new'
        );
        assert.ok(
            registeredCommands.includes('tmux-agents.new'),
            'tmux-agents.new should be registered'
        );
    });
});
...
suite('Command Execution — Safe Commands', () => {
    ...
    test('refresh executes without error', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.refresh'))
        );
    });

    test('toggleAutoRefresh executes without error (toggle on/off)', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.toggleAutoRefresh'))
        );
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.toggleAutoRefresh'))
        );
    ...
    test('configureServers executes without error', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.configureServers'))
        );
    });

    test('openDashboard executes without error', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.openDashboard'))
        );
    });

    test('openKanban executes without error', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.openKanban'))
        );
    });

    test('tmux-agents-chat.focus executes without error', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents-chat.focus'))
        );
    });
});
    ...
    for (const cmd of noArgCommands) {
        test(`${cmd} handles no arguments gracefully`, async () => {
            await assert.doesNotReject(
                ...
                `${cmd} should not throw when called without arguments`
            );
        });
    }
});
...
suite('Command Execution — QuickPick Commands', () => {
    ...
    test('killAgent executes without error (shows QuickPick, auto-dismisses)', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.killAgent'))
        );
    });

    test('sendToAgent executes without error (shows QuickPick, auto-dismisses)', async () => {
        await assert.doesNotReject(
            Promise.resolve(vscode.commands.executeCommand('tmux-agents.sendToAgent'))
        );
    });
});
...
suite('Tree View Providers', () => {
    ...
    test('tmux-agents sessions tree view is declared', () => {
        ...
        const sessionsView = views.find((v: any) => v.id === 'tmux-agents');
    ...
    test('tmux-agents-chat webview view is declared', () => {
        const views = pkg.contributes.views['tmux-agents'];
        const chatView = views.find((v: any) => v.id === 'tmux-agents-chat');
    ...
    test('tmux-agents-shortcuts tree view is declared', () => {
        const views = pkg.contributes.views['tmux-agents'];
        const shortcutsView = views.find((v: any) => v.id === 'tmux-agents-shortcuts');
    ...
    test('tmux-agents view container is declared in activity bar', () => {
        ...
        const container = containers.find((c: any) => c.id === 'tmux-agents');
...
suite('Keybindings', () => {
    test('All 16 expected keybinding commands are declared', () => {
        ...
        const declaredCommands = keybindings.map((kb: any) => kb.command);
        for (const cmd of expectedKeybindingCommands) {
            assert.ok(
                declaredCommands.includes(cmd),
                `Keybinding for '${cmd}' should be declared`
            );
        }
    });
});
...
suite('Activation Event + Menu Contributions', () => {
    ...
    test('onView:tmux-agents activation event is declared', () => {
        ...
        assert.ok(
            events.includes('onView:tmux-agents'),
            'Should include onView:tmux-agents activation event'
        );
    });
    ...
    test('view/title menus are declared and non-empty', () => {
        ...
        assert.ok(
            Array.isArray(menus['view/title']) && menus['view/title'].length > 0,
            'view/title menus should be non-empty'
        );
    });
    ...
    test('view/item/context menus are declared and non-empty', () => {
        ...
        assert.ok(
            Array.isArray(menus['view/item/context']) && menus['view/item/context'].length > 0,
            'view/item/context menus should be non-empty'
        );
    });
});
