...
import { OrganizationUnit, OrgUnitType } from './core/types';

function generateId(): string {
    return crypto.randomUUID?.() || 'id-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
}

export class OrganizationManager implements vscode.Disposable {
    private orgUnits: Map<string, OrganizationUnit> = new Map();

    private readonly _onOrgChanged = new vscode.EventEmitter<OrganizationUnit>();
    public readonly onOrgChanged: vscode.Event<OrganizationUnit> = this._onOrgChanged.event;

    // ─── CRUD ──────────────────────────────────────────────────────────────

    createOrgUnit(name: string, type: OrgUnitType, parentId?: string): OrganizationUnit {
    ...
    updateOrgUnit(id: string, updates: Partial<OrganizationUnit>): void {
        const existing = this.orgUnits.get(id);
        if (!existing) { throw new Error(`Org unit not found: ${id}`); }
    ...
    getRoots(): OrganizationUnit[] {
        return this.getAllOrgUnits().filter(u => !u.parentId);
    }

    getChildren(parentId: string): OrganizationUnit[] {
        return this.getAllOrgUnits().filter(u => u.parentId === parentId);
    }

    getAncestors(id: string): OrganizationUnit[] {
    ...
    getDescendants(id: string): OrganizationUnit[] {
    ...
    addMember(orgUnitId: string, agentId: string): void {
        const unit = this.orgUnits.get(orgUnitId);
        if (!unit) { throw new Error(`Org unit not found: ${orgUnitId}`); }
    ...
    removeMember(orgUnitId: string, agentId: string): void {
        const unit = this.orgUnits.get(orgUnitId);
        if (!unit) { return; }
        const idx = unit.memberIds.indexOf(agentId);
        if (idx !== -1) {
            unit.memberIds.splice(idx, 1);
            if (unit.leadAgentId === agentId) { unit.leadAgentId = undefined; }
            this._onOrgChanged.fire(unit);
        }
    }

    setLead(orgUnitId: string, agentId: string): void {
        const unit = this.orgUnits.get(orgUnitId);
        if (!unit) { throw new Error(`Org unit not found: ${orgUnitId}`); }
        unit.leadAgentId = agentId;
        if (!unit.memberIds.includes(agentId)) { unit.memberIds.push(agentId); }
        this._onOrgChanged.fire(unit);
    }

    findOrgUnitsForAgent(agentId: string): OrganizationUnit[] {
    ...
    getMergedContextInstructions(orgUnitId: string): string {
        const unit = this.orgUnits.get(orgUnitId);
        if (!unit) { return ''; }
        const parts: string[] = [];
        const ancestors = this.getAncestors(orgUnitId).reverse();
        for (const a of ancestors) {
            if (a.contextInstructions) { parts.push(a.contextInstructions); }
        }
        if (unit.contextInstructions) { parts.push(unit.contextInstructions); }
        return parts.join('\n');
    }

    // ─── Bulk Load ─────────────────────────────────────────────────────────

    loadOrgUnits(units: OrganizationUnit[]): void {
