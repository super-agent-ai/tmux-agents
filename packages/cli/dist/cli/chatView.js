...
const tmuxContextProvider_1 = require("./tmuxContextProvider");
...
function spawnWithStdin(command, args, input, timeoutMs = 60000, onSpawn, cwd) {
...
class ChatViewProvider {
    constructor(serviceManager, extensionUri, apiCatalog, contextDeps, aiManager) {
        ...
        this.conversationProcs = new Map();
        this.abortRequested = false;
        this.voiceProc = null;
        this.voiceTmpFile = '';
        // ── Conversation Persistence Events ─────────────────────────────────
        this._onConversationChanged = new vscode.EventEmitter();
        this.onConversationChanged = this._onConversationChanged.event;
        this._onConversationMessageAdded = new vscode.EventEmitter();
        this.onConversationMessageAdded = this._onConversationMessageAdded.event;
        this._onConversationDeleted = new vscode.EventEmitter();
        this.onConversationDeleted = this._onConversationDeleted.event;
        // ── Streaming CLI State ────────────────────────────────────────────
        this.selectedProvider = types_1.AIProvider.CLAUDE;
    ...
    switchConversation(convId) {
        ...
        if (target) {
            ...
            this.selectedProvider = target.aiProvider;
            this.selectedModel = (0, aiModels_1.resolveModelAlias)(target.model);
        }
        this.syncConversationBanners();
    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            if (msg.type === 'send') {
                ...
                this.selectedModel = (0, aiModels_1.resolveModelAlias)(msg.model);
            }
            else if (msg.type === 'setProvider') {
                this.selectedProvider = msg.provider;
                const models = this.getProviderModels();
    ...
    async transcribeAudio(audioPath) {
        ...
        for (const w of whisperCommands) {
            try {
                ...
                const result = cp.execSync(`${w.cmd} ${w.args.map(a => `"${a}"`).join(' ')}`, { timeout: 30000, encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] });
...
`;
    ...
    async scanFolder(rootPath, relativePath = '', depth = 0) {
        const entries = [];
        const treeLines = [];
        ...
        for (const item of items) {
            if (item.startsWith('.') && SKIP_DIRS.has(item)) {
                continue;
            }
    ...
    async readFolderFiles(rootPath, fileEntries) {
        ...
        const readable = fileEntries.filter(e => {
            ...
            return (TEXT_EXTENSIONS.has(ext) || TEXT_FILENAMES.has(basename)) && e.size < 500000;
        });
        ...
        for (const entry of readable) {
            if (filesRead >= MAX_FOLDER_FILES_READ) {
                break;
            }
            if (totalChars >= MAX_FOLDER_CHARS) {
                break;
            }
    ...
    async extractFileContent(filePath) {
        ...
        if (!ext && TEXT_FILENAMES.has(basename)) {
        ...
        if (['.docx', '.doc', '.rtf', '.odt'].includes(ext)) {
            return this.extractDocument(filePath);
        }
        return this.readAsTextWithFallback(filePath);
    }
    async readAsText(filePath) {
    ...
    async readAsTextWithFallback(filePath) {
        const buf = await readFile(filePath);
        if (buf.slice(0, 8192).includes(0)) {
            try {
                return await this.extractViaStrings(filePath);
            }
            catch {
                throw new Error('Binary file not supported. Use txt, pdf, docx, doc, rtf, odt, or code files.');
            }
        }
        const text = buf.toString('utf-8');
        return text.length > MAX_FILE_CHARS
    ...
    async extractDocument(filePath) {
        ...
        if (filePath.toLowerCase().endsWith('.docx')) {
            try {
                const { stdout } = await execAsync(`unzip -p '${escaped}' word/document.xml | sed -e 's/<[^>]*>//g' -e '/^$/d'`, { timeout: 15000, maxBuffer: 5 * 1024 * 1024 });
                const text = stdout.trim();
                if (text.length > 0) {
                    return text.length > MAX_FILE_CHARS
                        ? text.slice(0, MAX_FILE_CHARS) + `\n\n[... truncated]` : text;
    ...
    async extractViaStrings(filePath) {
        const escaped = filePath.replace(/'/g, "'\\''");
    ...
    // ── Provider Models ────────────────────────────────────────────────────
    getProviderModels() {
        return (0, aiModels_1.getModelsForProvider)(this.selectedProvider);
    }
    // ── System Prompt ───────────────────────────────────────────────────────
    async buildSystemPrompt() {
        const context = await (0, tmuxContextProvider_1.gatherFullExtensionContext)(this.serviceManager, this.contextDeps);
        const stateText = (0, tmuxContextProvider_1.formatFullContextForPrompt)(context);
...
9. **Listing vs. creating:** When the user asks to "see" or "show" something, use list/get actions. When they ask to "create", "add", or "set up", use create actions. Do not create resources when the user just wants to view them.`;
    ...
    // ── Streaming CLI Spawn ─────────────────────────────────────────────────
    spawnStreaming(prompt, onChunk, timeoutMs = 120000) {
        return new Promise((resolve, reject) => {
            ...
            const isAider = this.selectedProvider === types_1.AIProvider.AIDER;
            const isAmp = this.selectedProvider === types_1.AIProvider.AMP;
            const isCline = this.selectedProvider === types_1.AIProvider.CLINE;
            const isKiro = this.selectedProvider === types_1.AIProvider.KIRO;
            const isCursor = this.selectedProvider === types_1.AIProvider.CURSOR;
            const positionalPrompt = this.selectedProvider === types_1.AIProvider.COPILOT || isCline || isKiro || isCursor;
    ...
    async handleUserMessage(userText) {
        ...
        const userEntry = { role: 'user', content: userText, timestamp: Date.now() };
    ...
    async runAgentLoop() {
        for (let step = 0; step < MAX_AGENT_STEPS; step++) {
            ...
            const activeConv = this.getActiveConversation();
            if (activeConv) {
                const assistantEntry = { role: 'assistant', content: output, timestamp: Date.now() };
