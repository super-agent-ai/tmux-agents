...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function resolveService(deps, serverId) {
...
class ApiCatalog {
    ...
    tryParseJson(text) {
        try {
            ...
            if (parsed && typeof parsed === 'object' && !Array.isArray(parsed) && Array.isArray(parsed.actions)) {
                ...
                const explicitNext = ['tool', 'assistant', 'user'].includes(parsed.next) ? parsed.next : null;
                const next = explicitNext ?? (actions.length > 0 ? 'tool' : 'user');
    ...
    registerAISessionActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                const svc = resolveService(d, p.server);
                const launchCmd = d.aiManager.getLaunchCommand(p.provider);
    ...
    registerAIUtilityActions() {
        ...
        this.register({
            ...
            returnsData: true,
            execute: async (p) => {
                const status = d.aiManager.detectAIStatus(p.provider, p.content);
        ...
        this.register({
            ...
            returnsData: true,
            execute: async () => {
                const providers = Object.values(types_1.AIProvider).map(p => ({
                    provider: p,
    ...
    registerAgentActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!template) {
                    return err(`Template not found: "${p.templateId}"`);
                }
                const svc = resolveService(d, p.server);
                ...
                const agent = {
                    ...
                    state: types_1.AgentState.SPAWNING, serverId: svc.serverId,
                ...
                if (p.teamId) {
                    d.teamManager.addAgentToTeam(p.teamId, agentId);
                }
                // Transition to IDLE after brief delay
                setTimeout(() => d.orchestrator.updateAgentState(agentId, types_1.AgentState.IDLE), 3000);
                d.refreshTree();
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                const svc = d.serviceManager.getService(agent.serverId);
                if (!svc) {
                    return err(`Server not found: "${agent.serverId}"`);
                }
    ...
    registerAgentQueryActions() {
        ...
        this.register({
            ...
            returnsData: true,
            execute: async (p) => {
                const role = p.role ? p.role : undefined;
                const agents = d.orchestrator.getIdleAgents(role);
                if (agents.length === 0) {
                    return ok('No idle agents', []);
                }
        ...
        this.register({
            ...
            returnsData: true,
            execute: async (p) => {
                const agents = d.orchestrator.getAgentsByRole(p.role);
                if (agents.length === 0) {
                    return ok(`No agents with role "${p.role}"`, []);
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (agents.length === 0) {
                    return ok(`No agents in team "${p.teamId}"`, []);
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                d.orchestrator.updateAgentState(p.agentId, p.state, p.error);
    ...
    registerTeamActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!team) {
                    return err(`Team not found: "${p.teamId}"`);
                }
    ...
    registerTeamQueryActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!team) {
                    return ok(`Agent ${p.agentId} is not in any team`, { teamId: null });
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                const deps = (p.dependsOn && typeof p.dependsOn === 'string')
                    ? p.dependsOn.split(',').map((s) => s.trim()).filter(Boolean)
                    : [];
    ...
    registerPipelineQueryActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!pipeline) {
                    return err(`Pipeline not found: "${p.pipelineId}"`);
                }
                const stageSummary = pipeline.stages.map(s => `  "${s.name}" [${s.id}] type=${s.type} role=${s.agentRole}${s.dependsOn.length ? ` depends=[${s.dependsOn.join(',')}]` : ''}`);
        ...
        this.register({
            ...
            execute: async () => {
                ...
                if (runs.length === 0) {
                    return ok('No active pipeline runs', []);
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!run) {
                    return err(`Run not found: "${p.runId}"`);
                }
                ...
                const resultSummary = Object.entries(run.stageResults).map(([sid, r]) => `  "${stageNames[sid] || sid}": ${r.status}${r.agentId ? ` (agent: ${r.agentId})` : ''}${r.errorMessage ? ` ERROR: ${r.errorMessage}` : ''}`);
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (stages.length === 0) {
                    return ok('No stages ready to execute', []);
                }
    ...
    registerTemplateQueryActions() {
        ...
        this.register({
            ...
            returnsData: true,
            execute: async (p) => {
                const templates = d.templateManager.getTemplatesByRole(p.role);
                if (templates.length === 0) {
                    return ok(`No templates for role "${p.role}"`, []);
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (p.systemPrompt !== undefined) {
                    updates.systemPrompt = p.systemPrompt;
                }
    ...
    registerTaskQueryActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!task) {
                    return err(`Task not found: "${p.taskId}"`);
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (results.length === 0) {
                    return ok(`No completed results for stage ${p.stageId}`, []);
                }
    ...
    registerKanbanActions() {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                const task = {
                    ...
                    status: types_1.TaskStatus.PENDING, priority: p.priority || 5,
                    createdAt: Date.now(),
                    ...
                    dependsOn: p.dependsOn ? String(p.dependsOn).split(',').map((s) => s.trim()).filter((s) => s) : undefined,
            ...
            execute: async (p) => {
                ...
                if (p.column === 'done') {
                    ...
                    for (const t of allTasks) {
                        if (!t.dependsOn || !t.dependsOn.includes(p.taskId)) {
                            continue;
                        }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (p.swimLaneId) {
                    tasks = tasks.filter(t => t.swimLaneId === p.swimLaneId);
                }
                if (p.column) {
                    tasks = tasks.filter(t => t.kanbanColumn === p.column);
                }
                if (tasks.length === 0) {
                ...
                const summary = tasks.map(t => {
                    ...
                    if (t.output) {
                        line += ` summary="${t.output.slice(0, 60)}..."`;
                    }
                    return line;
                });
                return ok(`${tasks.length} task(s):\n${summary.join('\n')}`, tasks.map(t => ({
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (p.autoStart !== undefined) {
                    task.autoStart = !!p.autoStart;
                }
                if (p.autoPilot !== undefined) {
                    task.autoPilot = !!p.autoPilot;
                }
                if (p.autoClose !== undefined) {
                    task.autoClose = !!p.autoClose;
                }
                if (p.useWorktree !== undefined) {
                    task.useWorktree = !!p.useWorktree;
                }
                d.saveTask?.(task);
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!task.swimLaneId) {
                    return err('Task has no swim lane assigned. Assign a swim lane first.');
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (p.details !== undefined) {
                    task.input = p.details;
                }
                if (p.priority !== undefined) {
                    task.priority = p.priority;
                }
                if (p.role !== undefined) {
                    task.targetRole = p.role;
                }
                if (p.swimLaneId !== undefined) {
                    task.swimLaneId = p.swimLaneId || undefined;
                }
                if (p.autoStart !== undefined) {
                    task.autoStart = !!p.autoStart;
                }
                ...
                if (p.dependsOn !== undefined) {
                    const parsed = String(p.dependsOn).split(',').map((s) => s.trim()).filter((s) => s);
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!task.output) {
                    return ok('No completion summary available for this task.', { taskId: p.taskId, output: null });
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                return ok(`Task ${p.taskId}: "${task.description}"`, {
                    ...
                    autoStart: (0, types_1.resolveToggle)(task, 'autoStart', taskLane),
                    autoPilot: (0, types_1.resolveToggle)(task, 'autoPilot', taskLane),
                    autoClose: (0, types_1.resolveToggle)(task, 'autoClose', taskLane),
                    useWorktree: (0, types_1.resolveToggle)(task, 'useWorktree', taskLane),
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!lane) {
                    return err(`Swim lane not found: ${p.laneId}`);
                }
                if (p.name !== undefined) {
                    lane.name = p.name;
                }
                if (p.workingDirectory !== undefined) {
                    lane.workingDirectory = p.workingDirectory;
                }
                if (p.aiProvider !== undefined) {
                    lane.aiProvider = p.aiProvider || undefined;
                }
                if (p.contextInstructions !== undefined) {
                    lane.contextInstructions = p.contextInstructions || undefined;
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!lane.sessionActive) {
                    return ok('Session is already inactive');
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (task.tmuxSessionName && task.tmuxWindowIndex && task.tmuxServerId) {
                    ...
                    if (svc) {
                        try {
                            await svc.killWindow(task.tmuxSessionName, task.tmuxWindowIndex);
                        }
                        catch { }
                    }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!task.swimLaneId) {
                    return err('Task has no swim lane — cannot restart');
                }
                if (task.tmuxSessionName && task.tmuxWindowIndex && task.tmuxServerId) {
                    const oldSvc = d.serviceManager.getService(task.tmuxServerId);
                    if (oldSvc) {
                        try {
                            await oldSvc.killWindow(task.tmuxSessionName, task.tmuxWindowIndex);
                        }
                        catch { }
                    }
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                const svc = d.serviceManager.getService(task.tmuxServerId);
                if (!svc) {
                    return err(`Server "${task.tmuxServerId}" not found`);
                }
                try {
        ...
        this.register({
            ...
            returnsData: true,
            execute: async (p) => {
                const ids = p.taskIds.split(',').map((s) => s.trim()).filter(Boolean);
                const tasks = ids.map(id => d.orchestrator.getTask(id)).filter((t) => !!t);
                if (tasks.length < 2) {
                    return err('Need at least 2 valid tasks to merge');
                }
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                if (!parentTask) {
                    return err(`Parent task not found: ${p.parentTaskId}`);
                }
                if (!childTask) {
                    return err(`Child task not found: ${p.childTaskId}`);
                }
