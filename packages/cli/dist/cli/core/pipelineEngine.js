...
class PipelineEngine {
    constructor() {
        ...
        this._onPipelineStatusChanged = new eventEmitter_1.EventEmitter();
        this.onPipelineStatusChanged = this._onPipelineStatusChanged.event;
        this._onStageCompleted = new eventEmitter_1.EventEmitter();
    ...
    getReadyStages(run) {
        ...
        return pipeline.stages.filter(stage => {
            ...
            return stage.dependsOn.every(depId => {
                ...
                return depResult && depResult.status === types_1.TaskStatus.COMPLETED;
            });
        });
    }
    ...
    markStageCompleted(runId, stageId, output) {
        ...
        if (pipeline) {
            const allCompleted = pipeline.stages.every(stage => {
                ...
                return stageResult && stageResult.status === types_1.TaskStatus.COMPLETED;
            });
            if (allCompleted) {
                run.status = types_1.PipelineStatus.COMPLETED;
    ...
    markStageFailed(runId, stageId, error) {
        ...
        run.status = types_1.PipelineStatus.PAUSED;
        this._onPipelineStatusChanged.fire(run);
    }
    pauseRun(runId) {
        const run = this.activeRuns.get(runId);
    ...
    resumeRun(runId) {
        ...
        run.status = types_1.PipelineStatus.RUNNING;
        this._onPipelineStatusChanged.fire(run);
    }
    getActiveRuns() {
    ...
    generateTasksForStage(pipeline, stage, previousOutputs) {
        const tasks = [];
        const count = stage.type === types_1.StageType.FAN_OUT ? (stage.fanOutCount ?? 1) : 1;
        ...
        const codeReview = {
            ...
            stages: [
                ...
                },
                {
                    ...
                    name: 'Review',
                    type: types_1.StageType.SEQUENTIAL,
                    agentRole: types_1.AgentRole.REVIEWER,
        ...
        const fullDev = {
            ...
            stages: [
                ...
                },
                {
                    ...
                    name: 'Write Tests',
                    type: types_1.StageType.SEQUENTIAL,
                    agentRole: types_1.AgentRole.TESTER,
            ...
            stages: [
                {
                    ...
                    name: 'Research',
                    type: types_1.StageType.SEQUENTIAL,
                    agentRole: types_1.AgentRole.RESEARCHER,
...
Task: ${description}`;
            ...
            if (jsonMatch) {
                ...
                for (const s of stages) {
                    const role = Object.values(types_1.AgentRole).includes(s.role) ? s.role : types_1.AgentRole.CODER;
