...
export class K8sPool {
    ...
    async initialize() {
        ...
        try {
            ...
            await this.appsApi.replaceNamespacedDeployment(this.deploymentName, this.namespace, deploymentSpec);
        }
        catch (err) {
            if (err.response?.statusCode === 404) {
                // Deployment doesn't exist, create it
                await this.appsApi.createNamespacedDeployment(this.namespace, deploymentSpec);
            }
            else {
                throw err;
            }
    ...
    async claimPod(taskId) {
        ...
        try {
            await this.k8sApi.patchNamespacedPod(idlePod.name, this.namespace, {
                metadata: {
                    labels: {
                        'pool-claimed': 'true',
    ...
    async releasePod(podName) {
        // Remove claim labels
        await this.k8sApi.patchNamespacedPod(podName, this.namespace, {
            metadata: {
                labels: {
                    'pool-claimed': 'false',
    ...
    async scale(replicas) {
        ...
        await this.appsApi.patchNamespacedDeploymentScale(this.deploymentName, this.namespace, {
            spec: {
    ...
    async destroy() {
        try {
            await this.appsApi.deleteNamespacedDeployment(this.deploymentName, this.namespace);
        }
        catch (err) {
            if (err.response?.statusCode !== 404) {
                throw err;
