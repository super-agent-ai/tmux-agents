...
export class K8sRuntime implements AgentRuntime {
	...
	private config: K8sRuntimeConfig;
	...
	async listAgents(): Promise<AgentInfo[]> {
		...
		return response.items.map(pod => {
			...
			let state: AgentInfo['state'] = 'running';
			if (phase === 'Pending') {
				state = 'starting';
	...
	private generatePodName(taskId: string): string {
	...
	private getTmuxForPod(podName: string): TmuxService {
	...
	private buildPodSpec(podName: string, config: AgentConfig): k8s.V1Pod {
	...
	private buildResourceRequirements(
		...
		const requirements: k8s.V1ResourceRequirements = {
			requests: {},
			limits: {},
		};
	...
	private async waitForTmuxReady(
		tmux: TmuxService,
