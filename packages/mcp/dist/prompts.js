...
// ─── Prompt Handlers ───────────────────────────────────────────────────────
export async function handlePrompt(name, args, client) {
    switch (name) {
        case 'orchestrate':
...
async function handleOrchestratePrompt(args, client) {
    // Pre-fetch context
...
async function handleReviewProgressPrompt(args, client) {
    // Pre-fetch context
    const agents = await client.call('agent.list', {}).catch(() => []);
    ...
    const agentDetails = await Promise.all(agentsToShow.map(async (agent) => {
        try {
...
async function handleDebugStuckAgentPrompt(args, client) {
    // Pre-fetch context
    const agents = await client.call('agent.list', {}).catch(() => []);
