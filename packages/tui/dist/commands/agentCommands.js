...
import { AIProvider, AgentState, AgentRole } from '../types';
...
export function registerAgentCommands(context, ctx) {
    const disposables = [];
    ...
    disposables.push(vscode.commands.registerCommand('tmux-agents.manageTemplates', async () => {
        ...
        if (action.label === 'View Templates') {
            ...
            const rolePick = await vscode.window.showQuickPick(Object.values(AgentRole).map(r => ({ label: r })), { placeHolder: 'Agent role' });
            if (!rolePick) {
                return;
            }
            const providerPick = await vscode.window.showQuickPick(Object.values(AIProvider).map(p => ({ label: p })), { placeHolder: 'AI provider' });
            if (!providerPick) {
                return;
            }
