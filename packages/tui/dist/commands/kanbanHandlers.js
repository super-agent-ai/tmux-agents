...
import { TaskStatus, applySwimLaneDefaults, resolveToggle } from '../types';
...
export async function triggerDependents(ctx, completedTaskId) {
...
export async function handleKanbanMessage(action, payload, ctx) {
    switch (action) {
        case 'browseDir': {
        ...
        case 'approvePlan': {
            ...
            try {
                ...
                for (let i = 0; i < planTasks.length; i++) {
                    ...
                    if (approveLane) {
                        applySwimLaneDefaults(task, approveLane);
                    }
                    idMap.push(task.id);
