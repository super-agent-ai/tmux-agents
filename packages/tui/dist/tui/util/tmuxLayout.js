 ...
 * └─────────────────┴──────────────┘
 */
export function createTUISession(dashboardCommand, layout = DEFAULT_LAYOUT) {
    try {
        ...
        execSync(`tmux split-window -v -t ${TUI_SESSION_NAME}:0.1 -l ${layout.statusBarHeight}`, {
            encoding: 'utf-8',
        });
        // Set status bar to show resource usage
        execSync(`tmux send-keys -t ${TUI_SESSION_NAME}:0.2 "echo 'Resource Monitor - Press q in dashboard to exit'" C-m`, {
            encoding: 'utf-8',
        });
        // Set preview pane to show welcome message
        execSync(`tmux send-keys -t ${TUI_SESSION_NAME}:0.1 "echo 'Select an agent and press Enter to preview'" C-m`, {
            encoding: 'utf-8',
        });
        return getSessionPanes(TUI_SESSION_NAME);
    }
...
export function getSessionPanes(sessionName) {
    try {
        const output = execSync(`tmux list-panes -t ${sessionName}:0 -F "#{pane_id}:#{pane_index}"`, {
            encoding: 'utf-8',
        });
        return output
            .split('\n')
 ...
 */
export function attachToTUISession() {
    try {
 ...
 */
export function killTUISession() {
    try {
 ...
 */
export function getPreviewPaneId() {
    try {
...
//# sourceMappingURL=tmuxLayout.js.map