...
export class TmuxService {
    constructor(server = LOCAL_SERVER) {
        this.server = server;
        this.cache = null;
    ...
    async checkTmuxInstallation() {
        ...
        try {
            ...
            if (!this.server.isLocal && errorMessage.includes('Connection refused')) {
                vscode.window.showErrorMessage(`SSH connection refused${target}. Check that the host is reachable and SSH is running.`);
            }
            else if (!this.server.isLocal && errorMessage.includes('Permission denied')) {
                vscode.window.showErrorMessage(`SSH authentication failed${target}. Check your SSH key or credentials.`);
            }
            else if (!this.server.isLocal && errorMessage.includes('timed out')) {
                vscode.window.showErrorMessage(`SSH connection timed out${target}. The host may be unreachable.`);
            }
            else {
                vscode.window.showErrorMessage(`tmux is not installed or not in PATH${target}. Please install tmux to use this extension.`);
            }
            return false;
        }
    }
