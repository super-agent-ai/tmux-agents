// ─── Context Provider ────────────────────────────────────────────────────────
export async function gatherFullContext(serviceManager) {
    const servers = [];
...
export async function gatherFullExtensionContext(serviceManager, deps) {
...
// ─── Format for Prompt ───────────────────────────────────────────────────────
export function formatFullContextForPrompt(context) {
    const parts = [];
    // ── Tmux Servers ─────────────────────────────────────────────────────
        ...
        for (const p of context.pipelines) {
            ...
            for (const run of runs) {
                ...
                for (const [stageId, result] of Object.entries(run.stageResults)) {
                    ...
                    parts.push(`    "${stageName}": ${result.status}${result.agentId ? ` (agent: ${result.agentId})` : ''}`);
                }
            }
        }
    }
