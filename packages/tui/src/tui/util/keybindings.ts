...
const POPUP_BINDINGS: KeyBindingConfig[] = [
  {
    key: 'C-a',
 ...
 */
export function setupKeyBindings(_sessionName: string): void {
  for (const binding of POPUP_BINDINGS) {
    try {
      // Bind key to display popup with command
      execSync(
        `tmux bind-key -T root ${binding.key} display-popup -E -w 80% -h 80% "${binding.command}"`,
        {
          encoding: 'utf-8',
        }
      );
 ...
 */
export function openPopup(command: string, width = '80%', height = '80%'): void {
  try {
 ...
 */
export function openSendPromptPopup(agentId: string): void {
 ...
 */
export function openSpawnAgentPopup(): void {
 ...
 */
export function openSubmitTaskPopup(): void {
 ...
 */
export function openAgentPicker(): void {
 ...
 */
export function openTaskPicker(): void {
