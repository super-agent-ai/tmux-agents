...
// NOTE: This ProviderConfig is separate from the one in config.ts
// This one has required fields for runtime use after normalization
// The one in config.ts has optional fields for user configuration
...
export interface AIAssistantConfig {
    ...
    aiProviders?: Record<string, Partial<ProviderConfig>>;
    defaultWorkingDirectory?: string;
}

// Forward declaration for tmux service interface
export interface ITmuxService {
    newSession(sessionName: string): Promise<void>;
    sendKeys(session: string, windowIndex: number, paneIndex: number, keys: string): Promise<void>;
    capturePaneContent(session: string, windowIndex: number, paneIndex: number, lines: number): Promise<string>;
}
...
export class AIAssistantManager {
    ...
    public setConfig(config: AIAssistantConfig): void {
        this.config = config;
    }
    ...
    /**
     * Return the default AI provider from config.
     */
    getDefaultProvider(): AIProvider {
        const raw = this.config.defaultProvider || 'claude';
        return (Object.values(AIProvider).includes(raw as AIProvider) ? raw : 'claude') as AIProvider;
    }

    /**
     * Return the fallback AI provider from config.
     */
    getFallbackProvider(): AIProvider {
        const raw = this.config.fallbackProvider || 'gemini';
     ...
     * Priority: explicit override > lane provider > config default.
     */
    ...
    /**
     * Read provider config from config object.
     */
    ...
    private getProviderConfig(provider: AIProvider): ProviderConfig {
        const allProviders = this.config.aiProviders || {};
