...
// These haven't been moved to core yet - import from parent
...
export interface IAgentOrchestrator {
    ...
    sendMessage(fromAgentId: string, toAgentId: string, content: string): any;
    getUnreadMessages(agentId: string): any[];
    markMessageRead(messageId: string): void;
    getAllMessages(): any[];
    getConversation(agentId1: string, agentId2: string): any[];
    getFanOutResults(stageId: string): OrchestratorTask[];
    getIdleAgents(role?: any): AgentInstance[];
    getAgentsByRole(role: any): AgentInstance[];
    getAgentsByTeam(teamId: string): AgentInstance[];
    updateAgentState(agentId: string, newState: any, error?: string): void;
...
export interface ITeamManager {
    ...
    getTeamAgents(teamId: string): any[];
    setPipelineForTeam(teamId: string, pipelineId: string): void;
    findTeamForAgent(agentId: string): any | undefined;
}

export interface IPipelineEngine {
    getAllPipelines(): any[];
    createPipeline(name: string, description?: string): any;
    getPipeline(pipelineId: string): any | undefined;
    ...
    addStage(pipelineId: string, stage: any): any;
    removeStage(pipelineId: string, stageId: string): void;
    startRun(pipelineId: string): any;
    pauseRun(runId: string): void;
    resumeRun(runId: string): void;
    createPipelineFromDescription(description: string): any;
    getActiveRuns(): any[];
    ...
    getBuiltInPipelines(): any[];
    getReadyStages(runId: string): any[];
}
...
export interface ApiCatalogDeps {
    serviceManager: ITmuxServiceManager;
...
export function ok(message: string, data?: any): ApiActionResult {
    return { success: true, message, data };
}

export function err(message: string): ApiActionResult {
...
export class ApiCatalog {
    ...
    constructor(protected deps: ApiCatalogDeps) {
        ...
        // VS Code-specific actions are registered in subclass
    }
    ...
    protected register(action: ApiAction): void {
    ...
    private registerPipelineQueryActions(): void {
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                const stageSummary = pipeline.stages.map((s: any) =>
        ...
        this.register({
            ...
            execute: async (p) => {
                ...
                const stageNames = pipeline ? Object.fromEntries(pipeline.stages.map((s: any) => [s.id, s.name])) : {};
                const resultSummary = Object.entries(run.stageResults).map(([sid, r]: [string, any]) =>
    ...
    // VS Code-specific actions are registered in VS Code adapter subclass
    ...
    private registerKanbanActions(): void {
        ...
        this.register({
            ...
            returnsData: false,
            execute: async () => {
                // Note: In core, this just returns success. VS Code adapter can override to open UI.
                return ok('Kanban board functionality available');
            }
        });
        ...
        this.register({
            ...
            returnsData: false,
            execute: async () => {
                // Kanban board refresh happens via updateKanban callback
    ...
    // Quick actions (team.quickCoding, team.quickResearch) are registered in VS Code adapter
}
