...
import { EventEmitter, Event, Disposable } from './eventEmitter';
import type { TmuxService } from './tmuxService';

// Forward declaration - will be resolved by dependency injection
...
function evaluateAgentState(
    ...
    paneContent: string,
    aiAssistant: IAIAssistantManager
): AgentState {
...
export class AgentOrchestrator implements Disposable {
    private agents: Map<string, AgentInstance> = new Map();
    private taskQueue: OrchestratorTask[] = [];

    private serviceManager: ITmuxServiceManager | undefined;
    private aiAssistantManager: IAIAssistantManager;
    private pollingInterval: ReturnType<typeof setInterval> | undefined;

    private readonly _onAgentStateChanged = new EventEmitter<AgentInstance>();
    public readonly onAgentStateChanged: Event<AgentInstance> = this._onAgentStateChanged.event;

    private readonly _onTaskCompleted = new EventEmitter<OrchestratorTask>();
    public readonly onTaskCompleted: Event<OrchestratorTask> = this._onTaskCompleted.event;

    private readonly _onPipelineEvent = new EventEmitter<{ pipelineId: string; event: string }>();
    public readonly onPipelineEvent: Event<{ pipelineId: string; event: string }> = this._onPipelineEvent.event;

    private readonly _onAgentMessage = new EventEmitter<AgentMessage>();
    public readonly onAgentMessage: Event<AgentMessage> = this._onAgentMessage.event;
    ...
    public setServiceManager(sm: ITmuxServiceManager): void {
    ...
    public async checkAgentStates(serviceManager: ITmuxServiceManager): Promise<void> {
    ...
    public startPolling(serviceManager: ITmuxServiceManager, intervalMs: number): void {
        this.stopPolling();
