...
export class PipelineEngine implements vscode.Disposable {

    private pipelines: Map<string, Pipeline> = new Map();
    private activeRuns: Map<string, PipelineRun> = new Map();

    private _onPipelineStatusChanged = new vscode.EventEmitter<PipelineRun>();
    public readonly onPipelineStatusChanged: vscode.Event<PipelineRun> = this._onPipelineStatusChanged.event;

    private _onStageCompleted = new vscode.EventEmitter<{ runId: string; stageId: string; result: StageResult }>();
    public readonly onStageCompleted: vscode.Event<{ runId: string; stageId: string; result: StageResult }> = this._onStageCompleted.event;
    ...
    getBuiltInPipelines(): Pipeline[] {
        ...
        const researchImplement: Pipeline = {
            id: researchId,
    ...
    async createPipelineFromDescription(description: string): Promise<Pipeline> {
...
Task: ${description}`;
            ...
            if (jsonMatch) {
                const stages = JSON.parse(jsonMatch[0]) as Array<{name: string; role: string; type?: string; task: string; dependsOn?: string[]}>;
                const nameToId = new Map<string, string>();

                for (const s of stages) {
                    const role = Object.values(AgentRole).includes(s.role as AgentRole) ? s.role as AgentRole : AgentRole.CODER;
                    const added = this.addStage(pipeline.id, {
                        name: s.name,
                        type: StageType.SEQUENTIAL,
                        agentRole: role,
                        taskDescription: s.task,
                        dependsOn: (s.dependsOn || []).map(n => nameToId.get(n)).filter((id): id is string => !!id)
                    });
